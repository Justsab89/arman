...
//                                         }]
                    ...
                    if (err) throw err;
                        ...
                        connection.query(' CREATE TABLE ?? (id INT(100) NOT NULL AUTO_INCREMENT, id_report INT(11), id_user INT(11), date_entry DATETIME, product VARCHAR(100), size VARCHAR(100), ina3 INT(11), number INT(11), offprice INT(11), paper_type VARCHAR(100), paper_exp INT(11), A3_paper_exp INT(11), A4_paper_exp INT(11), paper_side VARCHAR(10), cut_exp INT(11), n_paper INT(11), PRIMARY KEY(id)) ',
...
//  var res2 = str.split("#");
  ...
  else if (res[0] =='sra3' || res[0] =='a3' || res[0] =='a4')  { insert_n_paper(query); bot.deleteMessage(query.message.chat.id, query.message.message_id) }
//  else if (res[0] =='sra3')  { show_result_sra3(query); bot.deleteMessage(query.message.chat.id, query.message.message_id) }
//  else if (res[0] =='a3')  { show_result_a3(query); bot.deleteMessage(query.message.chat.id, query.message.message_id) }
//  else if (res[0] =='a4')  { show_result_a4(query); bot.deleteMessage(query.message.chat.id, query.message.message_id) }
})
...
const text = '☑️ Чтобы ввести новый продукт наберите команду /product затем в этом порядке через # наберите 15 данных.\nНапример у вас следующие данные\n' +
             ...
             '\nТретье кол-во на SRА3: 8 ' +
             '\nЧетвертое кол-во на А3: 8 ' +
             '\nПятое кол-во на А4: 4 ' +
             '\nШестое себестоимость печати на струйном: 20 ' +
             '\nСедьмое наценка печати на струйном: 10 ' +
             '\nВосьмое себестоимость печати на ризографе: 10 ' +
             '\nДевятое наценка печати на ризографе: 5 ' +
             '\nДесятое себестоимость печати на офсете: 12 ' +
             ...
             '\nЧетвертое цену A4: 5' +
             '\nВ итоге вы вводите следующую команду' +
             '\n/paper толедо#10#10#5'
             '\n\n☑️ Чтобы ввести цену на резку наберите команду /cut затем в этом порядке через # наберите 3 данных' +
             '\nПервое цену за резку: 50' +
             '\nВторое кол-во ОТ: 0' +
             '\nТретье кол-во ДО: 100 ' +
             '\nВ итоге вы вводите следующую команду' +
             '\n/cut 50#0#100'
...
function show_result_sra3(query) {
...
pool.getConnection(function(err, connection) {
    ...
    var struyka_price = struyka[0].A3_price;
            ...
            var sql4 = ' SELECT product.name, product.size, product.A3_number AS ina3, product.print_exp, product.print_profit, product.cut_exp, product.cut_profit, ' +
                       ' product.offprint_exp, product.offprint_profit, product.digprint_exp, product.digprint_profit, ??.number AS number, ??.offprice AS offprice, ??.A3_paper_exp AS paper_exp, ??.paper_type, ??.paper_side ' +
                       ' FROM product JOIN ?? WHERE product.name = ??.product AND product.size = ??.size AND ??.size NOT LIKE "%*%" AND ??.id_report = (SELECT id_report FROM ?? ORDER BY id DESC LIMIT 1) ';
...
//' Имя: ' + nomer[0].username + ' номер: ' + nomer[0].tel + '\n' +
            ...
            console.log('ATEXXXT ',text)
             ...
             var sql5 = ' SELECT product.name AS name, product.A3_number AS ina3, product.print_exp AS print_exp, product.print_profit AS print_profit, ??.cut_exp AS cut_exp, ' +
                        ' product.offprint_exp AS offprint_exp, product.offprint_profit AS offprint_profit, product.digprint_exp AS digprint_exp, product.digprint_profit AS digprint_profit, ??.size AS size, ??.number AS number, ??.offprice AS offprice, ??.A3_paper_exp AS paper_exp, ??.paper_type AS paper_type, ??.paper_side AS paper_side ' +
                        ' FROM product JOIN ?? WHERE product.name = ??.product AND ??.size LIKE "%*%" AND product.size = "non" AND ??.id_report = (SELECT id_report FROM ?? ORDER BY id DESC LIMIT 1) ';
...
pool.getConnection(function(err, connection) {
    ...
    var struyka_price = struyka[0].A4_price;
            ...
            var sql4 = ' SELECT product.name, product.size, product.A4_number AS ina3, product.print_exp, product.print_profit, product.cut_exp, product.cut_profit, ' +
                       ' product.offprint_exp, product.offprint_profit, product.digprint_exp, product.digprint_profit, ??.number AS number, ??.offprice AS offprice, ??.A4_paper_exp AS paper_exp, ??.paper_type, ??.paper_side ' +
                       ' FROM product JOIN ?? WHERE product.name = ??.product AND product.size = ??.size AND ??.size NOT LIKE "%*%" AND ??.id_report = (SELECT id_report FROM ?? ORDER BY id DESC LIMIT 1) ';
...
//' Имя: ' + nomer[0].username + ' номер: ' + nomer[0].tel + '\n' +
            ...
            console.log('ATEXXXT ',text)
             ...
             var sql5 = ' SELECT product.name AS name, product.A4_number AS ina3, product.print_exp AS print_exp, product.print_profit AS print_profit, ??.cut_exp AS cut_exp, ' +
                        ' product.offprint_exp AS offprint_exp, product.offprint_profit AS offprint_profit, product.digprint_exp AS digprint_exp, product.digprint_profit AS digprint_profit, ??.size AS size, ??.number AS number, ??.offprice AS offprice, ??.A4_paper_exp AS paper_exp, ??.paper_type AS paper_type, ??.paper_side AS paper_side ' +
                        ' FROM product JOIN ?? WHERE product.name = ??.product AND ??.size LIKE "%*%" AND product.size = "non" AND ??.id_report = (SELECT id_report FROM ?? ORDER BY id DESC LIMIT 1) ';
...
// просчитываем для продукции со стандартным размером
            var sql4 = ' SELECT product.name, product.size, product.number AS ina3, product.print_exp, product.print_profit, product.cut_exp, product.cut_profit, ' +
                       ' product.offprint_exp, product.offprint_profit, product.digprint_exp, product.digprint_profit, product.rizprint_exp, product.rizprint_profit, ' +
                       ' ??.number AS number, ??.offprice AS offprice, ??.paper_exp AS paper_exp, ??.A3_paper_exp AS A3_paper_exp, ??.A4_paper_exp AS A4_paper_exp, ??.paper_type, ??.paper_side, ??.n_paper ' +
                       ' FROM product JOIN ?? WHERE product.name = ??.product AND product.size = ??.size AND ??.size NOT LIKE "%*%" AND ??.id_report = (SELECT id_report FROM ?? ORDER BY id DESC LIMIT 1) ';
...
//            console.log('ATEXXXT ',text)
             ...
             connection.query( sql5 , [order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table, order_table], function(err, rows, fields) {
             if (err) throw err;
             var counting = JSON.parse(JSON.stringify(rows));
             console.log('НЕСТАНДАРт ', counting);
